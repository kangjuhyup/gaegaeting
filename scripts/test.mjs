...
if (args.length === 0) {
  ...
  if (services.length === 0) {
    ...
    console.log('📖 사용법: pnpm test [서비스명...] [--parallel] [--watch]');
    console.log('📖 예시: pnpm test account match --parallel');
    process.exit(1);
  }
}
...
if (isParallel) {
  ...
  const concurrentlyCommands = services.map(service => 
    `"pnpm --filter ${service} ${testCommand}"`
  );
  ...
  async function testSequentially() {
    for (const service of services) {
      ...
      const child = spawn('pnpm', ['--filter', service, 'test'], {
        stdio: 'inherit',
        shell: true,
        cwd: projectRoot
      });
