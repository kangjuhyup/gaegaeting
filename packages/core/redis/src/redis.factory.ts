...
import type { RedisClientBuildOptions } from './redis.type.js';
...
export function createRedisClient(opts: RedisClientBuildOptions): Redis | Cluster {
  ...
  if (mode === 'multi-single') {
    ...
    return new Redis(first, opts.client.options);
  }
  ...
  if (mode === 'sentinel') {
    ...
    return new Redis({ sentinels, name: opts.client.name, ...(opts.client.options ?? {}) });
  }
...
export function createRedisClients(opts: RedisClientBuildOptions): Array<Redis | Cluster> {
  ...
  if (mode === 'multi-single') {
    ...
    return opts.client.urls.map((u) => new Redis(u, opts.client.options));
  }
