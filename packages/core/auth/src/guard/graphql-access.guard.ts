import { type ExecutionContext, ForbiddenException, Injectable, UnauthorizedException } from '@nestjs/common';
...
export class GraphqlAccessGuard {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const assertion = req.headers?.['x-gaegaeting-principal'];
      if (typeof assertion !== 'string' || assertion === '') throw new Error('missing');
      req.user = this.auth.verify(assertion);
